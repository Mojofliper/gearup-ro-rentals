...
export const checkPermission = async (
  ...
  return userPermissions?.[resource as keyof typeof userPermissions]?.includes(action) || false
}
```
  ...
  static async detectSuspiciousActivity(userId: string, action: string): Promise<boolean> {
    try {
      ...
      if (payments.length > 20) {
        await SecurityLogger.logSuspiciousActivity(userId, 'excessive_payments')
        return true
      }
