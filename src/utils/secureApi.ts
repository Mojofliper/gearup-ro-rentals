  ...
  try {
    ...
    if (options?.rateLimit) {
      try {
        ...
        console.warn(
          "Rate limit check failed, allowing operation to proceed:",
          rateLimitError,
        );
      }
    }
    ...
    console.log(
      `Secure API call successful${options?.rateLimit ? ` - ${options.rateLimit.action}` : ""}`,
    );
    ...
    // Log security event
    console.error(
      `Secure API call failed${options?.rateLimit ? ` - ${options.rateLimit.action}` : ""}:`,
      error,
    );
    throw error;
  }
