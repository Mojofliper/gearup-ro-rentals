...
export const GearCard = React.memo<GearCardProps>(({ 
  ...
  const getFirstImage = () => {
    if (gear.gear_photos && Array.isArray(gear.gear_photos) && gear.gear_photos.length > 0) {
      ...
      if (firstPhoto && typeof firstPhoto === 'object') return firstPhoto.photo_url || firstPhoto;
    }
    return null;
  };
    ...
    >
      ...
      <div className="flex-1 flex flex-col justify-between p-4 gap-2">
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm text-gray-700 mb-1">
          ...
          {isOwnerVerified && <CheckCircle className="h-4 w-4 text-blue-500 ml-1" />}
        ...
        <div className="flex flex-wrap items-center gap-4 mb-2">
          <span className="flex items-center gap-1 text-base sm:text-lg font-bold text-blue-700">
            <span className="text-xl sm:text-2xl">{formatPrice(gear.price_per_day)}</span>
            <span className="text-xs font-medium text-gray-500 ml-1">/ zi</span>
          </span>
        ...
        <div className="flex flex-col gap-2 mt-2 sm:flex-row sm:justify-end">
          {onBookNow && isAvailable && (
            <Button
              onClick={e => { e.stopPropagation(); onBookNow(gear.id); }}
