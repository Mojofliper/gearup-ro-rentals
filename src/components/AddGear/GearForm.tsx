    ...
    try {
      const gearData = {
        ...
        deposit_amount: formData.depositAmount
          ? Math.round(parseFloat(formData.depositAmount))
          : 0,
        pickup_location: formData.pickupLocation
          ? sanitizeInput(formData.pickupLocation)
          : "Romania",
        ...
        specifications:
          formData.specifications.length > 0 ? formData.specifications : [],
        included_items:
          formData.includedItems.length > 0 ? formData.includedItems : [],
        is_available: true,
      };
  ...
  const Stepper = () => (
    <div className="sticky top-0 z-20 bg-white/80 backdrop-blur-sm py-3 mb-6 rounded-xl shadow-sm flex items-center justify-center gap-6 sm:gap-10">
      <div className="flex flex-col items-center">
        <div
          className={`w-9 h-9 flex items-center justify-center rounded-full border-2 ${currentStep === 1 ? "border-blue-600 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-400"} font-bold text-lg transition-all`}
        >
          <Info className="h-5 w-5" />
        </div>
        <span
          className={`mt-1 text-xs font-medium ${currentStep === 1 ? "text-blue-700" : "text-gray-400"}`}
        >
          Detalii
        </span>
      </div>
      <div
        className={`h-1 w-10 sm:w-16 rounded-full ${currentStep === 2 ? "bg-blue-600" : "bg-gray-200"}`}
      ></div>
      <div className="flex flex-col items-center">
        <div
          className={`w-9 h-9 flex items-center justify-center rounded-full border-2 ${currentStep === 2 ? "border-blue-600 bg-blue-50 text-blue-700" : "border-gray-200 bg-white text-gray-400"} font-bold text-lg transition-all`}
        >
          <Camera className="h-5 w-5" />
        </div>
        <span
          className={`mt-1 text-xs font-medium ${currentStep === 2 ? "text-blue-700" : "text-gray-400"}`}
        >
          Fotografii
        </span>
      </div>
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[40vh] py-12 animate-fade-in">
        ...
        <p className="text-gray-600 mb-4">
          Redirecționare către pagina echipamentului...
        </p>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto w-full bg-white/90 rounded-2xl shadow-xl p-2 sm:p-6 md:p-8">
      ...
      <div className="space-y-8">
        ...
        {currentStep === 2 && (
          <div className="bg-white rounded-xl shadow-sm p-4 sm:p-6 border border-gray-100">
            <h3 className="text-lg sm:text-xl font-bold text-gray-900 mb-4 flex items-center gap-2">
              <ImagePlus className="h-5 w-5 text-blue-600" /> Fotografii
              echipament
            </h3>
