...
import {
  ...
  SearchFilters as AdvancedSearchFiltersType,
} from "./BrowseGear/AdvancedSearchFilters";
import {
  Sheet,
  SheetContent,
  SheetHeader,
...
export const BrowseGear: React.FC = () => {
  ...
  const {
    ...
    isLoading,
    error,
  } = useAllGear() as { data: Gear[]; isLoading: boolean; error: unknown };
  ...
  const filteredGear = useMemo(() => {
    ...
    if (filters.category && filters.category !== "all") {
      filtered = filtered.filter(
        (item) => item.categories?.slug === filters.category,
      );
    }
      ...
      filtered = filtered.filter((item) => {
        const price =
          typeof item.price_per_day === "number"
            ? item.price_per_day > 1000
              ? item.price_per_day / 100
              : item.price_per_day
            : 0;
    ...
    if (filters.location && filters.location !== "all") {
      filtered = filtered.filter((item) =>
        (item.pickup_location?.toLowerCase() || "").includes(
    ...
    switch (filters.sortBy) {
      case "price-low":
        filtered.sort((a, b) => {
          const priceA =
            typeof a.price_per_day === "number"
              ? a.price_per_day > 1000
                ? a.price_per_day / 100
                : a.price_per_day
              : 0;
          const priceB =
            typeof b.price_per_day === "number"
              ? b.price_per_day > 1000
                ? b.price_per_day / 100
                : b.price_per_day
              : 0;
      ...
      case "rating":
        filtered.sort(
          (a, b) => (b.users?.rating || 0) - (a.users?.rating || 0),
        );
        break;
      default:
  ...
  const handleBasicFilterChange = useCallback(
    (
      key: keyof AdvancedSearchFiltersType,
  ...
  const handleAdvancedFiltersChange = useCallback(
    (newFilters: AdvancedSearchFiltersType) => {
  ...
  const MobileFilterSheet = () => (
    <Sheet open={showMobileFilters} onOpenChange={setShowMobileFilters}>
      <SheetContent
        side="bottom"
        className="max-h-[90vh] overflow-y-auto rounded-t-2xl p-4"
      >
        <SheetHeader>
      ...
      <div className="min-h-screen bg-gradient-to-br from-white via-slate-50 to-gray-100">
        ...
        <div className="block sm:hidden px-4 pt-4 pb-2">
          ...
          <div className="flex flex-col gap-2 mt-4">
            <Button className="w-full text-base font-semibold bg-blue-700 hover:bg-blue-800 text-white shadow rounded-xl py-3">
              Închiriază echipament
            </Button>
            <Button
              variant="outline"
              className="w-full text-base font-semibold border-gray-300 text-blue-700 shadow rounded-xl py-3"
            >
        ...
        <div
          id="gear-results"
          className="container mx-auto px-0 sm:px-4 py-2 sm:py-8"
        >
          {/* Results Header */}
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-2 sm:mb-6 gap-4 px-2 sm:px-0">
            <div>
              <h2 className="text-lg sm:text-3xl font-bold text-gray-900 mb-2">
                {isLoading
                  ? "Se încarcă..."
                  : `${filteredGear.length} echipament${filteredGear.length !== 1 ? "e" : ""} găsit${filteredGear.length !== 1 ? "e" : ""}`}
              </h2>
            </div>
          </div>
          {/* Content */}
              ...
              <CardContent>
                <div className="max-w-md mx-auto">
                  ...
                  <p className="text-gray-600 mb-6">
                    Nu am putut încărca echipamentele. Te rugăm să încerci din
                    nou.
                  </p>
