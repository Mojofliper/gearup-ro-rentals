...
export const GearDetail: React.FC = () => {
  ...
  const { data: unavailableDates, isLoading: unavailableLoading } =
    useGearUnavailableDates(id);
        ...
        <div className="container mx-auto px-4 py-16">
          <div className="text-center">
            ...
            <p className="text-gray-600 mb-8">
              Ne pare rău, echipamentul căutat nu există sau a fost șters.
            </p>
            <Button
              asChild
  ...
  const owner = gearWithRelations.users as
    | {
        full_name?: string;
        location?: string;
        is_verified?: boolean;
    ...
    | undefined;
  const images =
    (gear.gear_photos as { photo_url: string }[] | undefined)?.map(
      (photo) => photo.photo_url,
    ) || [];
  const specifications: string[] = Array.isArray(gear.gear_specifications)
    ? (
        gear.gear_specifications as { spec_key: string; spec_value: string }[]
      ...
      if (error instanceof Error && error.name === "QuotaExceededError") {
        try {
          ...
          toast({
            ...
            description:
              "Nu s-a putut adăuga echipamentul în coș. Încearcă din nou.",
            variant: "destructive",
          });
        }
      } else {
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
        ...
        <div className="relative w-full bg-black">
          ...
          <Button
            ...
            variant="ghost"
            size="icon"
            className="absolute top-4 left-4 z-20 bg-white/80 hover:bg-white rounded-full shadow-md"
          >
            <Link to="/browse">
          ...
          <Button
            variant="ghost"
            size="icon"
            className="absolute top-4 right-4 z-20 bg-white/80 hover:bg-white rounded-full shadow-md"
          >
              ...
              <div className="flex items-center gap-4 mt-2">
                <span className="text-2xl font-bold text-blue-700">
                  {new Intl.NumberFormat("ro-RO", {
                    ...
                    minimumFractionDigits: 0,
                    maximumFractionDigits: 0,
                  }).format(Number(gear.price_per_day))}
                </span>
                <span className="text-gray-500 text-sm">/ zi</span>
                <span className="flex items-center gap-1 text-xs text-gray-500">
                  <Package className="h-4 w-4" /> Depozit:{" "}
                  <span className="font-semibold ml-1">
                    {new Intl.NumberFormat("ro-RO", {
                      ...
                      minimumFractionDigits: 0,
                      maximumFractionDigits: 0,
                    }).format(Number(gear.deposit_amount))}
                  </span>
                </span>
              </div>
            </div>
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-500">
                      ...
                      <span className="flex items-center gap-1">
                        ...
                        {owner?.total_reviews || 0} recenzii
                      </span>
                    </div>
                  </div>
                </CardContent>
              ...
              {reviews.length > 0 ? (
                <div className="space-y-4">
                  {reviews.slice(0, 3).map((review, idx) => (
                    <div
                      key={
                        String((review as { id?: string | number }).id) + idx
                      }
                    ...
                    >
                      ...
                      <p className="text-gray-700">
                        {String((review as { comment?: string }).comment || "")}
                      </p>
                    </div>
                  ))}
                ...
                <p className="text-gray-500">
                  Nu există recenzii pentru acest echipament.
                </p>
              )}
            </div>
          </div>
                      ...
                      <span className="font-medium">
                        {(() => {
                          ...
                          return new Intl.NumberFormat("ro-RO", {
                            ...
                            minimumFractionDigits: 0,
                            maximumFractionDigits: 0,
                          }).format(days * Number(gear.price_per_day));
                        })()}
                      </span>
                    </div>
                    <div className="flex justify-between items-center">
                    ...
                    <div className="flex justify-between items-center font-semibold text-lg">
                      ...
                      <span className="text-blue-600">
                        {(() => {
                          ...
                          const days = daysDiff === 0 ? 1 : daysDiff + 1;
                          const total =
                            days * Number(gear.price_per_day) +
                            Number(gear.deposit_amount);
                          return new Intl.NumberFormat("ro-RO", {
                            style: "currency",
                            currency: "RON",
                            minimumFractionDigits: 0,
