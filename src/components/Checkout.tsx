  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      const bookingPromises = cartItems.map(async (item) => {
        const sortedDates = [...item.selectedDates].sort(
        ...
        if (bookingError) {
          throw new Error(
            `Failed to create booking for ${item.gear.title}: ${bookingError.message}`,
          );
        }
        ...
        if (booking && booking.id) {
          try {
            ...
            if (
              ...
              isNaN(transaction.rental_amount) ||
              isNaN(transaction.deposit_amount) ||
              isNaN(transaction.platform_fee)
            ) {
            ...
            setPaymentError(
              `Eroare la plata pentru ${item.gear.title}: ${errorMessage}`,
            );
            break;
          }
        }
      ...
      const errorMessage =
        error instanceof Error
          ? error.message
          : "Nu s-a putut procesa plata. Te rugăm să încerci din nou.";
        ...
        <DialogContent className="max-w-md">
          <div className="text-center py-8">
            ...
            <p className="text-gray-600 mb-4">
              Rezervările au fost create cu succes. Vei fi redirecționat către
              plata securizată.
            </p>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                  <div className="flex justify-between text-sm">
                    ...
                    <span>
                      {formatAmountForDisplay(calculateRentalTotal())}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Total garanții</span>
                    <span>
                      {formatAmountForDisplay(calculateDepositTotal())}
                    </span>
                  </div>
                  <div className="flex justify-between text-sm">
                    <span>Taxă platformă (13%)</span>
                    <span>
                      {formatAmountForDisplay(calculatePlatformFeeTotal())}
                    </span>
                  </div>
                  <Separator />
          ...
          <div className="space-y-4">
                ...
                <div className="flex items-start space-x-2">
                  ...
                  <div className="text-sm">
                    <p className="font-medium text-green-800">
                      Plată securizată cu escrow
                    </p>
                    <p className="text-green-700">
                      Toate plățile sunt procesate prin Stripe și protejate prin
                      sistemul de escrow. Fondurile sunt eliberate automat după
            ...
            <p className="text-xs text-gray-500 text-center">
              Prin finalizarea comenzii, ești de acord cu termenii și condițiile
              GearUp.
            </p>
          </div>
        </div>
