...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    if (isLoading) {
      loadingTimeoutRef.current = setTimeout(() => {
        if (mountedRef.current) {
          console.log(
            "AuthProvider: Loading timeout reached, stopping loading",
          );
          setLoading(false);
        }
  ...
  const fetchProfile = async (userId: string) => {
    if (!userId || !mountedRef.current) {
      console.log(
        "AuthProvider: fetchProfile called with invalid userId or unmounted component:",
        { userId, mounted: mountedRef.current },
      );
      setProfile(null);
      setLoadingWithTimeout(false);
      return;
  ...
  useEffect(() => {
    ...
    const {
      ...
      if (!initialLoadComplete) {
        console.log(
          "AuthProvider: Skipping auth state change during initial load:",
          event,
        );
        return;
      }
      ...
      if (event === "SIGNED_IN" && session?.user) {
        ...
        if (!profile || profile.id !== session.user.id) {
          ...
          try {
            ...
            console.error(
              "AuthProvider: Error fetching profile on sign in:",
              error,
            );
            setLoadingWithTimeout(false);
          }
          ...
          console.log(
            "AuthProvider: Profile already exists for user, skipping fetch",
          );
        }
      } else if (event === "SIGNED_OUT") {
        console.log("AuthProvider: User signed out");
        console.log("AuthProvider: Current state before sign out:", {
          user: !!user,
          profile: !!profile,
          loading,
        });
    ...
    const handleVisibilityChange = async () => {
      ...
      if (document.visibilityState === "visible" && user && !profile) {
        ...
        try {
          ...
          console.error(
            "AuthProvider: Error checking session on visibility change:",
            error,
          );
        }
      }
    ...
    const handleWindowFocus = async () => {
      ...
      if (user && !profile && !loading) {
        ...
        try {
          ...
          if (session?.user) {
            console.log(
              "AuthProvider: Session still valid on focus, fetching profile",
            );
            setLoadingWithTimeout(true);
            await fetchProfile(session.user.id);
          } else {
            console.log(
              "AuthProvider: No valid session on focus, clearing state",
            );
            setUser(null);
            setProfile(null);
            setLoadingWithTimeout(false);
          ...
          console.error(
            "AuthProvider: Error checking session on focus:",
            error,
          );
    ...
    try {
      ...
      if (error) {
        if (error.message.includes("Invalid login credentials")) {
          return {
            error:
              "Email sau parolă incorectă. Te rugăm să verifici datele și să încerci din nou.",
          };
        } else if (error.message.includes("Email not confirmed")) {
          return {
            error:
              "Contul nu a fost confirmat. Te rugăm să verifici emailul și să confirmi contul.",
          };
        } else if (error.message.includes("Too many requests")) {
          return {
            error:
              "Prea multe încercări de conectare. Te rugăm să aștepți câteva minute înainte de a încerca din nou.",
          };
        } else {
      ...
      try {
        ...
        console.error(
          "AuthProvider: Error fetching profile after login:",
          profileError,
        );
        setLoadingWithTimeout(false);
      ...
      setTimeout(async () => {
        if (mountedRef.current && user?.id === data.user.id && !profile) {
          console.log(
            "AuthProvider: Fallback profile fetch for user:",
            data.user.id,
          );
          try {
            await fetchProfile(data.user.id);
            ...
            console.error(
              "AuthProvider: Fallback profile fetch failed:",
              error,
            );
          }
        }
    ...
    try {
      ...
      if (error) {
        if (error.message.includes("User already registered")) {
          return {
            error:
              "Un cont cu acest email există deja. Te rugăm să te conectezi sau să resetezi parola.",
          };
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          ...
          description:
            "Nu s-a putut actualiza profilul. Te rugăm să încerci din nou.",
          variant: "destructive",
        });
        return;
      }
  ...
  React.useEffect(() => {
    ...
    if (!storageAvailable) {
      toast({
        ...
        description:
          "Stocarea locală este dezactivată. Autentificarea și sesiunea nu vor funcționa corect în modul incognito sau dacă browserul blochează stocarea.",
        variant: "destructive",
        duration: 10000,
      });
    }
