...
export const useEscrowPayments = () => {
  ...
  const [connectedAccount, setConnectedAccount] =
    useState<ConnectedAccountStatus | null>(null);
  const [escrowTransaction, setEscrowTransaction] =
    useState<EscrowTransaction | null>(null);

  // Setup Stripe Connect account for gear owner
  const setupStripeConnect = useCallback(
    async (email: string, country: string = "RO") => {
      if (!user) {
      ...
      setLoading(true);
      try {
        const result = await PaymentService.setupStripeConnect(
          user.id,
          email,
          country,
        );
    ...
    async (params: {
      ...
      try {
        ...
        const { data: fullBooking, error: bookingError } = await supabase
          .from("bookings")
          .select(
            "id, rental_amount, deposit_amount, total_amount, start_date, end_date, gear:gear(title)",
          )
